...
def contenus_ajouter_wtf():
    ...
    if request.method == "POST":
        try:
            ...
            if form.validate_on_submit():
                ...
                strsql_select_last_contenu = """SELECT id_contenu FROM T_contenu ORDER BY id_contenu DESC"""

                with MaBaseDeDonnee() as mconn_bd:
                    mconn_bd.mabd_execute(strsql_select_last_contenu)

                    nomDeVariable = mconn_bd.fetchone()
                    last_id_contenu = nomDeVariable["id_contenu"]
                    print("last_id", last_id_contenu)

                # Ici, j'essaie de faire la liason de la fk contenu et fk piece en récuperant la dernière id crée
                strsql_insert_fk = """INSERT INTO t_avoir_contenu(Id_avoir_contenu, Fk_contenu, Fk_piece) VALUES %(value_Nom_piece)s),%(value_Fk_contenu)s)"""


                with MaBaseDeDonnee() as mconn_bd:
                    mconn_bd.mabd_execute(strsql_insert_fk, valeurs_insertion_dictionnaire)
    ...
    with MaBaseDeDonnee().connexion_bd.cursor() as afficher:
        strsql_piece_afficher = """SELECT t_piece.Nom_piece, t_piece.Id_piece FROM t_piece ORDER BY Id_piece ASC"""


        afficher.execute(strsql_piece_afficher)

        piece = afficher.fetchall()
